...
import { InvoiceData, InvoiceItem, WeightEntry, CustomExpense } from '../types';
  ...
  return (
    <div className="bg-white p-3 sm:p-5 md:p-8 rounded-[1.5rem] sm:rounded-[2.5rem] shadow-2xl space-y-6 sm:space-y-10 border border-gray-100">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center border-b border-emerald-50 pb-6 gap-4 sm:gap-6">
        <div>
          <h2 className="text-2xl sm:text-3xl font-black text-emerald-900 tracking-tight urdu-font">?? ?????? <span className="text-emerald-500 font-sans">Pro</span></h2>
          <p className="text-xs sm:text-sm text-emerald-600 font-bold opacity-80 mt-1">???? ??? ??? ??????? ??? ????</p>
        </div>
        <button onClick={onScan} className="w-full md:w-auto bg-emerald-600 hover:bg-emerald-700 text-white px-6 py-3 sm:px-8 sm:py-4 rounded-2xl flex items-center justify-center gap-3 transition-all shadow-lg active:scale-95 group">
          ...
          <span className="font-black text-base sm:text-lg">AI ??????</span>
        </button>
      </div>

      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-4 sm:gap-6">
        <div className="space-y-1.5">
          <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest block px-1">?? ????</label>
        ...
        <div className="space-y-1.5">
          <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest block px-1">????? ?? ???</label>
          <input type="text" className="w-full rounded-xl border-gray-100 p-3 border font-bold text-sm text-right focus:border-emerald-50" value={data.partyName} onChange={(e) => updateField('partyName', e.target.value)} />
        </div>
        <div className="space-y-1.5">
          <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest block px-1">????? ?? ???</label>
          <input type="text" className="w-full rounded-xl border-gray-100 p-3 border font-bold text-sm text-right focus:border-emerald-50 bg-emerald-50/5" value={data.brokerName} onChange={(e) => updateField('brokerName', e.target.value)} />
        </div>
        <div className="space-y-1.5">
          <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest block px-1">???? ????</label>
          <input type="text" className="w-full rounded-xl border-gray-100 p-3 border font-bold text-sm text-right focus:border-emerald-50" value={data.trolleyNo} onChange={(e) => updateField('trolleyNo', e.target.value)} />
        </div>
        <div className="space-y-1.5">
          <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest block px-1">?????</label>
      ...
      <div className="space-y-4 sm:space-y-6 bg-gradient-to-tr from-emerald-50/40 to-white p-4 sm:p-6 rounded-2xl border border-emerald-100">
        <div className="flex justify-between items-center">
          <h3 className="text-lg sm:text-xl font-black text-emerald-900 urdu-font">????? ??? ???</h3>
          <button onClick={addItem} className="text-emerald-700 font-black text-xs bg-white px-4 py-2 rounded-xl border shadow-sm">+ ????</button>
        </div>
          ...
          {data.items.map((item, idx) => (
            <div key={item.id} className="grid grid-cols-1 md:grid-cols-12 gap-3 items-end bg-white p-3 rounded-2xl border border-emerald-50 shadow-sm">
              <div className="md:col-span-3">
                <label className="text-[9px] font-black text-gray-400 mb-1 block">????? (Item)</label>
                <input className="w-full rounded-lg border-gray-100 p-2 font-bold text-sm text-right" value={item.description} onChange={(e) => {
                  const items = [...data.items]; items[idx].description = e.target.value; updateField('items', items);
                }} />
              </div>
              <div className="md:col-span-1">
                <label className="text-[9px] font-black text-gray-400 mb-1 block text-center">??? (Bags)</label>
              ...
              <div className="md:col-span-2">
                <label className="text-[9px] font-black text-emerald-600 mb-1 block text-center">??? (Gross Weight)</label>
              ...
              <div className="md:col-span-2">
                <label className="text-[9px] font-black text-red-500 mb-1 block text-center">??????? ???/?????</label>
              ...
              <div className="md:col-span-2">
                <label className="text-[9px] font-black text-emerald-600 mb-1 block text-center">??? (Rate)</label>
              ...
              <div className="md:col-span-2 flex justify-between items-center p-2">
                <div className="text-right">
                   <span className="text-[8px] block font-black text-gray-400">???</span>
      ...
      <div className="bg-gray-50 p-4 sm:p-6 rounded-[2rem] border border-gray-200">
        <h3 className="text-lg font-black text-gray-800 mb-4 urdu-font">??????? ??? ??????? (Deductions)</h3>
        <div className="grid grid-cols-2 md:grid-cols-5 gap-4 mb-6">
           <div className="space-y-1">
             <label className="text-[10px] font-black text-gray-500 block">????? (%)</label>
           ...
           <div className="space-y-1">
             <label className="text-[10px] font-black text-gray-500 block">?????? (Labor)</label>
        ...
        <div className="space-y-3 pt-4 border-t border-gray-200">
           <div className="flex justify-between items-center">
             <h4 className="text-sm font-black text-gray-600">???? ??????? (Extra Charges)</h4>
             <button onClick={addCustomExpense} className="text-[10px] bg-white border px-3 py-1 rounded-lg font-black hover:bg-gray-100">+ ???? ???? ????</button>
           </div>
           {data.customExpenses && data.customExpenses.map((exp, idx) => (
             <div key={exp.id} className="flex gap-3 items-center">
               <input className="flex-1 p-2 border rounded-lg text-sm text-right" placeholder="???? ?? ???" value={exp.name} onChange={(e) => {
                 const exps = [...data.customExpenses]; exps[idx].name = e.target.value; updateField('customExpenses', exps);
               }} />
               <input type="number" className="w-32 p-2 border rounded-lg text-sm text-center" placeholder="???" value={exp.amount || ''} onChange={(e) => {
      ...
      <div className="bg-emerald-900 text-white p-6 rounded-3xl shadow-xl space-y-6">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-center">
            <div><span className="text-[9px] opacity-60 block uppercase">????? ???</span><span className="text-xl font-black">{finalGrossWeight} ???</span></div>
            <div><span className="text-[9px] opacity-60 block uppercase">??????? ?????</span><span className="text-xl font-black text-red-300">-{totalKattWeight} ???</span></div>
            <div><span className="text-[9px] opacity-60 block uppercase">???? ???</span><span className="text-xl font-black">{activeNetWeight} ??? ({(activeNetWeight/40).toFixed(2)} ??)</span></div>
            <div><span className="text-[9px] opacity-60 block uppercase">?? ???????</span><span className="text-xl font-black text-red-300">Rs {totalDeductions.toLocaleString(undefined, {maximumFractionDigits:0})}</span></div>
          </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <button onClick={onNewBill} className="p-5 border-2 border-emerald-100 text-emerald-700 font-black rounded-3xl hover:bg-emerald-50">??? ??</button>
        <button onClick={onPrint} className="p-5 bg-emerald-600 text-white font-black rounded-3xl shadow-lg hover:bg-emerald-700">???? ????</button>
      </div>
    </div>
  );
};
