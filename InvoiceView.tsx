...
const InvoiceView: React.FC<Props> = ({ data }) => {
  ...
  return (
    <div className="print-container bg-white p-4 sm:p-6 rounded-3xl shadow-xl border-t-8 border-gray-800 mx-auto max-w-[800px] print:max-w-full print:p-2 print:m-0 print:shadow-none print:border-none print:rounded-none overflow-hidden font-sans rtl text-center" dir="rtl">
      
      {/* Header */}
      <div className="mb-4 pb-2 border-b-2 border-dashed border-black/20 print:border-black/50 text-center space-y-1">
        <h1 className="text-3xl sm:text-5xl font-black text-gray-900 mb-2 print:text-xl urdu-text leading-tight">{data.shopName || 'مل / شاپ کا نام'}</h1>
        <p className="text-lg sm:text-xl font-bold text-gray-700 print:text-[11px] urdu-text">{data.address || 'ایڈریس درج نہیں ہے'}</p>
        <div className="text-gray-900 font-black text-xl print:text-[13px] urdu-text">
          <span>فون: {data.phone || 'فون نمبر'}</span>
        </div>
      </div>

      {/* Bill Info */}
      <div className="mb-4 print:mb-2 text-base sm:text-xl print:text-[13px] urdu-text border-b border-black/10 pb-2 flex justify-center gap-4">
      ...
      {/* Party Details */}
      <div className="text-center space-y-1 mb-4 print:mb-3 urdu-text text-lg sm:text-2xl print:text-[14px]">
          <p className="font-bold">پارٹی کا نام: <span className="font-black border-b border-dotted border-gray-300">{data.partyName || '---'}</span></p>
          {data.brokerName && <p className="font-bold">بروکر: <span className="font-black border-b border-dotted border-gray-300">{data.brokerName}</span></p>}
      ...
      <div className="border-y-2 border-dashed border-black/40 py-2 mb-4 print:mb-2 text-center">
        <div className="grid grid-cols-3 gap-1 text-[12px] sm:text-xl print:text-[13px] urdu-text">
        ...
        {data.items.map((item, idx) => {
          ...
          return (
            <div key={idx} className="border-b border-dashed border-black/10 py-4 print:py-2">
              <div className="flex justify-center items-center gap-3 text-lg sm:text-2xl print:text-[14px] font-black">
                 <span>{item.quantity} {item.description}</span>
                 <span>وزن: <span>{itemNetWeight.toFixed(2)}k</span></span>
              </div>
              <div className="mt-1">
                 <span className="text-2xl sm:text-3xl print:text-[18px] font-black block py-1">
                    Rs {itemAmount.toLocaleString(undefined, {maximumFractionDigits: 0})} /-
                 </span>
                 <p className="text-xs sm:text-lg print:text-[11px] opacity-70">ریٹ: Rs {item.rate} فی من</p>
              </div>
            </div>
          );
        })}
      ...
      <div className="grid grid-cols-2 gap-2 text-right urdu-text text-sm sm:text-xl print:text-[12px] border-t border-black/10 pt-4">
          
          {/* ADDITIONS سیکشن (+) */}
          <div className="border-l border-dashed border-black/20 pl-1">
      ...
      <div className="text-center urdu-text py-2">
        <p className="text-base sm:text-xl print:text-[13px] font-bold opacity-70 mb-1">صافی قابلِ ادائیگی رقم</p>
        <p className="text-4xl sm:text-6xl font-black print:text-[26px] text-gray-900">Rs {netPayable.toLocaleString(undefined, {maximumFractionDigits: 0})}</p>
      </div>
      ...
      <div className="mt-6 print:mt-4 text-center urdu-text">
        <div className="flex justify-between gap-4 px-2 text-base sm:text-xl print:text-[13px] font-black pb-8">
        ...
        <p className="text-[11px] print:text-[10px] italic font-black pt-4 border-t border-dashed border-black/10 opacity-40">
