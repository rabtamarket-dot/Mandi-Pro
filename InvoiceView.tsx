...
const InvoiceView: React.FC<Props> = ({ data }) => {
  ...
  return (
    <div className="print-container bg-white p-4 sm:p-6 md:p-8 rounded-3xl shadow-xl border-t-8 border-gray-800 mx-auto max-w-[800px] print:max-w-[100%] print:p-0 print:shadow-none print:border-none print:rounded-none overflow-hidden font-sans rtl" dir="rtl">
      ...
      {/* Header - Centered for Receipt */}
      <div className="text-center mb-4 pb-2 print:pb-1 border-b-2 border-dashed border-black/10 print:border-black">
        <h1 className="text-2xl sm:text-4xl font-black text-gray-900 mb-1 tracking-tighter print:text-lg print:uppercase">{data.shopName || '?? / ??? ?? ???'}</h1>
        <p className="text-sm sm:text-lg font-bold text-gray-700 mb-1 print:text-[9px] print:leading-tight">{data.address || '?????? ??? ???? ??'}</p>
        <div className="flex items-center justify-center gap-2 text-gray-900 font-black text-lg print:text-[10px]">
          <span>???: {data.phone || '??? ????'}</span>
        </div>
      </div>

      {/* Info Section - Stacked for narrow thermal */}
      <div className="grid grid-cols-2 gap-x-2 mb-2 print:mb-2 text-xs sm:text-base print:text-[10px] leading-tight">
        <div className="space-y-0.5 text-right">
          <p className="font-bold">?? #: <span className="font-black">{data.billNumber || '---'}</span></p>
          <p className="font-bold">?????: <span className="font-black">{data.date}</span></p>
          {data.brokerName && <p className="font-bold">?????: <span className="font-black">{data.brokerName}</span></p>}
        </div>
        <div className="text-right space-y-0.5 border-r border-black/5 pr-2 print:border-black/20">
          <p className="font-bold">?????: <span className="font-black">{data.partyName || '---'}</span></p>
          <p className="font-bold">????: <span className="font-black">{data.trolleyNo || '---'}</span></p>
        </div>
      </div>

      {/* Dashed Separator for Thermal */}
      <div className="hidden print:block border-t border-dashed border-black my-2"></div>

      {/* WEIGHT SUMMARY - Optimized for Receipt */}
      <div className="bg-emerald-50/50 p-3 rounded-2xl mb-4 border border-emerald-100 print:bg-transparent print:p-0 print:border-none print:mb-2">
        <div className="grid grid-cols-3 gap-1 text-center text-xs sm:text-sm print:text-[9px]">
          <div className="flex flex-col">
            <span className="font-bold opacity-60 print:opacity-100">?? ???</span>
          ...
          <div className="flex flex-col">
            <span className="font-bold opacity-60 print:opacity-100">???? ???</span>
            <div className="flex flex-col">
              <span className="font-black text-emerald-700 print:text-black">{maunds} ?? {kgs > 0 ? `${kgs} ???` : ''}</span>
              <span className="text-[8px] print:text-[7px] font-bold">({activeNetWeight} ???)</span>
            </div>
          </div>
        </div>
      </div>
      ...
      {/* Items Table - Fixed to show Net Weight */}
      <div className="mb-4 print:mb-2 overflow-x-hidden">
        <table className="w-full text-right text-[10px] sm:text-sm print:text-[9px] border-collapse">
          <thead>
            <tr className="border-b border-gray-800 bg-gray-50 print:bg-transparent print:border-black">
              <th className="py-1 px-0.5 text-center font-black">???</th>
              <th className="py-1 px-0.5 font-black text-right">????</th>
              <th className="py-1 px-0.5 text-center font-black">???? ???</th>
              <th className="py-1 px-0.5 text-center font-black">???</th>
              <th className="py-1 px-0.5 text-left font-black">???</th>
            </tr>
          </thead>
          <tbody>
      ...
      {/* Deductions - Traditional Receipt Style */}
      <div className="space-y-1 text-[10px] sm:text-sm print:text-[9px] mb-2">
          ...
          {data.khaliBardanaRate > 0 && (
            <div className="flex justify-between items-center">
              <span>???? ??????? (@{data.khaliBardanaRate}):</span>
      ...
      {/* Net Payable - Bold & Centered */}
      <div className="bg-gray-900 text-white p-3 rounded-2xl text-center print:bg-transparent print:text-black print:p-0">
        <p className="text-[9px] font-bold uppercase tracking-widest opacity-70 print:opacity-100">?? ???? ???????</p>
      ...
      {/* Footer / Signature */}
      <div className="mt-4 print:mt-2 text-center text-[9px] print:text-[8px] font-bold opacity-60 print:opacity-100 space-y-4">
        <div className="flex justify-between gap-4 px-2">
           <div className="flex-1 border-t border-black/20 pt-1">????? ?? ?????</div>
           <div className="flex-1 border-t border-black/20 pt-1">?????? {data.shopName}</div>
        </div>
        <p className="pt-2 italic">Mandi Bill Pro ???? ???? ?? ???? ????</p>
      </div>
    </div>
  );
};
